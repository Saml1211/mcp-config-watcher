...
async function testJsonRpcDiscovery() {
  try {
    // Load configuration
    ...
    config.discovery.timeout = 20000; // 20 seconds
    ...
    if (!serverId) {
      ...
      console.log(`Example: node test-rpc-discovery.js "github.com/some/server"`);
      process.exit(0);
    }
    ...
    console.log(`\nTesting JSON-RPC discovery for server: ${serverId}`);
...
testJsonRpcDiscovery().catch(error => {
